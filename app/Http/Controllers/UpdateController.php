...
class UpdateController extends Controller
{
    public function step0(Request $request) {
        if (env('DEMO_MODE') == 'On') {
            flash(('This action is disabled in demo mode'))->error();
            return back();
        }
        if ($request->has('update_zip')) {
            if (class_exists('ZipArchive')) {
                ...
                if ($res === true) {
                    ...
                    flash(('Could not open the updates zip file.'))->error();
                    return back();
                }
                ...
                flash(('Please enable ZipArchive extension.'))->error();
            }
        }
        else {
    ...
    public function step1() {
        if(get_setting('current_version') == '5.5.1'){
