...
@section('content')
        ...
        <div class="container mobile_no_px">
            <div class="row gutters-10 position-relative mobile_no_mx">
                ...
                <div class="mt-2 col-lg-{{  $banner_class }} banner-size-{{ $banner_class }}">
                    ...
                    @if ((@get_setting('home_slider_images') != null) || (@get_setting('home_slider_images_mobile') != null))
                        <div class="aiz-carousel dots-inside-bottom mobile-img-auto-height" data-arrows="true" data-dots="true" data-autoplay="true">
                            @php 

                                $slider_images_mobile = json_decode(@get_setting('home_slider_images_mobile'), true);  
                                $slider_images_web = json_decode(@get_setting('home_slider_images'), true); 
                                ...
                                if($agent->isMobile()){  
                                    ...
                                    if(isAllNull($slider_images_mobile)){
                                        $slider_images = $slider_images_web;
                                    }else{
                                        $slider_images = $slider_images_mobile;
                                    };
    ...
    {{-- add show start--}}
    @if ($ads = ads_show('below_slider'))
        @includeIf('ekattor.frontend.ads',['ads'=> $ads])
    @endif
    {{-- add show end --}}
                ...
                @foreach ($featured_categories as $key => $category)
                    <li class="minw-0 col-3 col-md-2 mb-0 px-0 border border-width-1 bg-white text-center">
                        <a href="{{ route('products.category', $category->slug) }}" class="d-block rounded bg-white-ex p-1 text-reset shadow-sm py-2">
                            <img
                                src="{{ static_asset('assets/img/placeholder.jpg') }}"
                                data-src="{{ uploaded_asset($category->featured_icon) }}"
        ...
        <div class="container">
            <div class="px-2 py-2 px-md-2 py-md-2 bg-white shadow-sm rounded section_holder">
                <div class="d-flex flex-wrap mb-3 align-items-baseline border-bottom section_title_holder sale_title">
                    ...
                    <a href="{{ route('flash-deal-details', $flash_deal->slug) }}" class="ml-auto mr-0 btn btn-primary btn-sm shadow-md w-100 w-md-auto view_btn-ex">{{ translate('View More') }}</a>
                </div>
                    ...
                    @foreach ($flash_deal->flash_deal_products->take(20) as $key => $flash_deal_product)
                        ...
                        @if ($product != null && $product->published != 0)
                            <div class="carousel-box px-1">
                                @include(config('app.theme').'frontend.partials.product_box_1',['product' => $product,'flash_deal_countdown'=>false])
                            </div>
                        @endif
                    @endforeach
                </div>
