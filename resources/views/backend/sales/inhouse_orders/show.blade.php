    	...
    	<div class="card-body">
            ...
            <div class="row gutters-5 mt-5">
                ...
                <div class="col text-center">
                    <span style="font-size:22px; font-weight:bold; color:@if($order->payment_status=='unpaid') red @else black @endif">{{ strtoupper($order->payment_status) }}</span>
                </div>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    @elseif ($orderDetail->product != null && $orderDetail->product->auction_product == 1)
                                        <a href="{{ to_frontend(route('auction-product', $orderDetail->product->slug)) }}" target="_blank"><img height="50px" src="{{ uploaded_asset($orderDetail->product->thumbnail_img) }}"></a>
                                    @else
                                      <strong>{{ ('N/A') }}</strong>
                                    @endif
                                </td>
    		...
    		<div class="clearfix float-left">
    			<table class="table">
          			<tbody>
                        ...
                        <tr>
                            <td>
                                <strong class="text-muted">{{ ('Coupon')}} @if(@$order->orderDetails[0]->coupon_code!=NULL) ({{ $order->orderDetails[0]->coupon_code }}) @endif :</strong>
                            </td>
                            <td>
                        ...
                        <tr>
                            ...
                            <td class="text-muted h5">
                                {{ single_price($order->orderDetails->sum('price') - $order->coupon_discount) }}
                            </td>
                        </tr>
                        @endif
            			<tr>
            			...
            			<tr>
            				<td><strong class="text-muted">{{ ('NET TOTAL')}} :</strong></td>
            				<td class="text-muted h5">
            					{{ single_price($order->grand_total) }}
            				</td>
            			</tr>
