...
const SortBySelectSearchOption = () => {
  const { sort_by, setSearchFilter } = useSearchStore();
  const [isPending, startTransition] = useTransition();
  const isMobile = useMediaQuery("(max-width: 768px)");

  const handleSortChange = (value: string) => {
    startTransition(() => {
      setSearchFilter("sort_by", value);
      setSearchFilter("page", 1); // reset page on sort change
    });
  };
...
export default SortBySelectSearchOption;
