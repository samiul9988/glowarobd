...
use Illuminate\Support\Facades\{DB, Log, Auth, Cache, Session};
...
class OrderController extends Controller
{
    ...
    private function applyFilters(Builder $query, array $filters): Builder
    {
        ...
        Debugbar::info('Order Query', ['query' => $query->toSql(), 'bindings' => $query->getBindings()]);
        return $query;
    }
    ...
    public function bulk_product_download() {
        $status = $_GET['status'];
    ...
    public function store(Request $request)
    {
        ...
        $carts = Cart::withoutGlobalScopes()->where('user_id', Auth::id())->get();
                    ...
                    if($date->format('w') == intval($day[$i])){
                        ...
                        foreach ($seller_products as $seller_product) {
                            ...
                            foreach ($seller_product as $cartItem) {
                                ...
                                $cartItem['price'] = getMinimumPriceByVariant($product, $product_stock, 'web', $cartItem['quantity'], $this->currentlyAuthenticatedUser);
                                ...
                                if (!check_preorder_product($product)) {
                                    if ($product->digital != 1 && $cartItem['quantity'] > $product_stock->qty && $product->allow_stock_out_purchases == 0) {
                                        ...
                                        if ($flash_deal_check) {
                                            ...
                                            $flash_deal_product->quantity -= $cartItem['quantity'];
            ...
            if(isset($orderIds) && count($orderIds) > 0){
                foreach($orderIds as $order){
                    try {
                        ...
                        $systemAreaID = ($orderData->order_source == 'POS') ? $orderData->address_id : Address::find($orderData->address_id)->area_id;
                                ...
                                if(isset($shippingMethod) && $shippingMethod->id === 1){
                                    ...
                                    if(!isset($pathaoMatchedArea)){
                                        ...
                                        if($pathaoAreaInfo){
                                            ...
                                            if(isset($res['type']) && $res['type'] == 'error' && count($res['errors']) > 0){
                                                $newArray = array_values($res['errors']);
                                                $result = [];
