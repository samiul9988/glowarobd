...
class CategoryUtility
{
    ...
    public static function get_immediate_children($id, $with_trashed = false, $as_array = false)
    {
        ...
        $children = Cache::remember('uni_category_children'.$id, 86400, function() use($id){
    ...
    public static function get_immediate_children_ids($id, $with_trashed = false)
    {
        ...
        $children = Cache::remember('get_immediate_children_ids_'.$id, 86400, function () use ($id, $with_trashed) {
            return CategoryUtility::get_immediate_children($id, $with_trashed, true);
        });
    ...
    {
        $category = Cache::remember('category_'.$id, now()->addHours(3), function() use ($id) {
        ...
        $children_ids = Cache::remember('category_children_ids_'.$id, now()->addHours(3), function() use ($category) {
            return self::children_ids($category->id);
        });

        $count = Cache::remember('category_products_count_'.$id, now()->addHours(3), function() use ($category, $children_ids) {
            $all_ids = array_merge([$category->id], $children_ids ?? []);
            return \App\Models\Product::whereIn('category_id', $all_ids)->published()->count();
        });
        return $count ?? 0;
    }
}
