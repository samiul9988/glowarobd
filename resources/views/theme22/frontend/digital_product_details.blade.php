                            ...
                            <div class="d-table width-100 mt-3">
                                <div class="d-table-cell">
                                    ...
                                    @if(Auth::check() && addon_is_activated('affiliate_system') && (\App\Models\AffiliateOption::where('type', 'product_sharing')->first()->status || \App\Models\AffiliateOption::where('type', 'category_wise_affiliate')->first()->status) && Auth::user()->affiliate_user != null && Auth::user()->affiliate_user->status)
                                        ...
                                        <button type=button id="ref-cpurl-btn" class="btn btn-sm btn-secondary" data-attrcpy="{{ ('Copied')}}" onclick="CopyToClipboard('referral_code_url')">{{ ('Copy the Promote Link')}}</button>
                                    @endif
                                </div>
                            </div>
        ...
        <div class="modal-dialog modal-dialog-centered modal-dialog-zoom product-modal" id="modal-size" role="document">
            <div class="modal-content position-relative">
                <div class="modal-header">
                    <h5 class="modal-title fw-600 heading-5">{{ ('Any question about this product?')}}</h5>
...
@section('script')
    <script type="text/javascript">
        let relatedProductUrl = "{{ route('products.related', ':id') }}".replace(':id', '{{ $detailedProduct->id }}');
        $.get(relatedProductUrl, function(data) {
            $('#related-products').html(data).fadeIn(400, function(){
                // AIZ.plugins.slickCarousel();
            });
        });


        let commentableUrl = "{{ route('products.isCommentable', ':id') }}".replace(':id', '{{ $detailedProduct->id }}');
        $.get(commentableUrl, function(data) {
