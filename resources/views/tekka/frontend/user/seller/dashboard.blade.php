    ...
    <div class="row">
        ...
        <div class="col-md-5">
          <div class="card p-5 text-center">
              ...
              @if(Auth::user()->seller->verification_status == 0)
                  <a href="{{ route('shop.verify') }}" class="btn btn-primary">{{ ('Verify Now')}}</a>
              @endif
          </div>
        </div>
    </div>
    ...
    <div class="row">
          ...
          <div class="card">
              <div class="card-header">
                ...
                <div class="text-center">
                    <a href="{{ route('seller.products.upload')}}" class="btn btn-primary d-inline-block">{{ ('Add New Product')}}</a>
                </div>
              </div>
          </div>
      </div>
      <div class="col-md-4">
              ...
              <div class="card">
                  ...
                  <div class="card-body text-center">
                      @if($seller_package != null)
                        ...
                        <p class="mb-1 text-muted">{{ ('Product Upload Remaining') }}: {{ Auth::user()->seller->remaining_uploads }} {{ ('Times')}}</p>
                        <p class="text-muted mb-1">{{ ('Digital Product Upload Remaining') }}: {{ Auth::user()->seller->remaining_digital_uploads }} {{ ('Times')}}</p>
                        @if(addon_is_activated('auction'))
                        <p class="text-muted mb-1">{{ ('Auction Product Upload Remaining') }}: {{ Auth::user()->seller->remaining_auction_uploads }} {{ ('Times')}}</p>
                        @endif
                        <p class="text-muted mb-4">{{ ('Package Expires at') }}: {{ Auth::user()->seller->invalid_at }}</p>
                        <h6 class="fw-600 mb-3 text-primary">{{ ('Current Package') }}: {{ $seller_package->name }}</h6>
                      @else
                          <h6 class="fw-600 mb-3 text-primary">{{ ('Package Not Found')}}</h6>
                      @endif
                      <div class="text-center">
                          <a href="{{ route('seller_packages_list') }}" class="btn btn-soft-primary">{{ ('Upgrade Package')}}</a>
                      </div>
                  </div>
              </div>
          @endif
          <div class="card mb-4 p-4 text-center">
              ...
              <a href="{{ route('shops.index') }}" class="btn btn-soft-primary">{{ ('Go to setting')}}</a>
          </div>
          <div class="card mb-4 p-4 text-center">
              <div class="h5 fw-600">{{ ('Payment')}}</div>
              <p>{{ ('Configure your payment method')}}</p>
              <a href="{{ route('profile') }}" class="btn btn-soft-primary">{{ ('Configure Now')}}</a>
          </div>
      </div>
    </div>
