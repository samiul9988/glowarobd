...
const VariantSelector = ({
  ...
  const safeColors = colorOptions || {};
  const safeChoices = choiceOption || [];
  const allColor = Object.keys(safeColors);
  ...
  useEffect(() => {
    if (safeChoices.length) {
      ...
      safeChoices.forEach((item) => {
        if (item?.options?.length > 0) {
          defaults[item.name] = item.options[0];
        }
      });
      setSelectedOptions(defaults);
      onVariantChange?.({ color: safeColors[selectedColor], ...defaults });
    }
  }, [safeChoices, safeColors, selectedColor]);

  const handleOptionChange = (name: string, value: string) => {
    setSelectedOptions((prev) => {
      const updated = { ...prev, [name]: value };
      onVariantChange?.({ color: safeColors[selectedColor], ...updated });
      return updated;
    });
  };

  if (allColor.length === 0 && safeChoices.length === 0) {
    return null;
  }
