...
class RouteServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        ...
        Route::macro('allModules', function (string $routeFile = 'api') {
            try {
                foreach(Module::allEnabled() as $module) {
                    $routePath = $module->getPath() . "/Routes/{$routeFile}.php";
                ...
                Log::error("Error loading all module routes: " . $e->getMessage());
                return;
            }
        });

        Route::macro('module', function (string $moduleName, string $routeFile = 'api') {
            try {
                ...
                Log::error("Error loading module routes for {$moduleName}: " . $e->getMessage());
                return;
            }
        });
    }
    ...
    {
        // Route::middleware(['web'])
        Route::middleware(['rewrite_url', 'web'])
            ->namespace($this->namespace)
    ...
    {
        Route::prefix('api/merchant/v1')
        // ->middleware(['web', 'api'])
            ->middleware(['web', 'merchant.api.logger'])
