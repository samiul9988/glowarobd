...
<div class="aiz-card-box border border-2 rounded-md hov-shadow-md mt-1 mb-1 has-transition bg-white product_box">
    @if(discount_in_percentage($product) > 0)
        <div class="ribbon"><span class="ribbon4"><span>{{ ('OFF') }}&nbsp;{{discount_in_percentage($product)}}%</span></span></div>
    @endif
    <div class="d-flex d-md-block">
        ...
        <div class="pt-3 pb-0 pb-md-1 text-center product_info minw-60 pl-1 pr-2">
            ...
            <div id="app-price" class="fs-15 text-left text-primary fw-700 ml-2 d-block mb-1 pt-1 pb-md-0">
                <span>
                    @if(getMinimumPriceByVariant($product, $product->stocks->first(), 'app', 1, $currentlyAuthenticatedUser) < getMinimumPriceByVariant($product, $product->stocks->first(), 'web', 1, $currentlyAuthenticatedUser))
                        App Price: {{ single_price(getMinimumPriceByVariant($product, $product->stocks->first(), 'app', 1, $currentlyAuthenticatedUser)) }}
                    @else
                        &nbsp;
                    @endif
                </span>
