...
<div class="row mb-2">
    <div class="col">
        <h5 class="mb-md-0 h5">{{ ('All notices') }}</h5>
    </div>
    <div class="col">
        <a href="{{ route('notices.create') }}" class="btn btn-success btn-sm float-right">
            <i class="las la-plus"></i> {{ ('Create Notice') }}
        </a>
    </div>
</div>
    <div class="card">
                ...
                <div class="col-lg-2">
                    <div class="form-group mb-0">
                        <select class="form-control aiz-selectpicker" name="category" data-live-search="true">
                            ...
                            @foreach (\App\Models\NoticeCategory::pluck('name', 'id') as $id => $name)
                ...
                <div class="col-lg-2">
                    <div class="form-group mb-0">
                        <select class="form-control" name="visibility">
                            ...
                            <option value="customers" @if(@$visibility == 'customers') selected @endif>{{ ('Customers') }}</option>
                            <option value="staffs" @if(@$visibility == 'staffs') selected @endif>{{ ('Staffs') }}</option>
                            <option value="both" @if(@$visibility == 'both') selected @endif>{{ ('Both') }}</option>
                        </select>
                    </div>
                </div>
                <div class="col-lg-2">
                ...
                <table class="table aiz-table mb-0">
                    <thead>
                        <tr>
                            ...
                            <th data-breakpoints="md">{{ ('Visibility') }}</th>
                            <th data-breakpoints="md">{{ ('Publish Date') }}</th>
                    ...
                    <tbody>
                        @foreach ($notices as $key => $notice)
                            <tr>
                                ...
                                <td>
                                    {{ Str::limit($notice->category->name, 50) }}
                                </td>
                                <td>
                                    @php
                                        $class = match($notice->status) {
                                            'draft' => 'secondary',
                                            'published' => 'success',
                                            'scheduled' => 'warning',
                                            default => 'secondary',
                                    ...
                                    @endphp
                                    <span class="badge badge-inline badge-{{ $class }} font-weight-bold">{{ ucfirst(translate($notice->status)) }}</span>
                                </td>
                                <td>
                                    @php
                                        $class = match($notice->visibility) {
                                            'both' => 'success',
                                            'customers' => 'secondary',
                                            'staffs' => 'info',
                                            default => 'success',
                                    ...
                                    @endphp
                                    <span class="badge badge-inline badge-{{ $class }} font-weight-bold">{{ ucfirst(translate($notice->visibility)) }}</span>
                                </td>
                                <td>
                                    @if ($notice->publish_at)
                                        {{ $notice->publish_at->format('d-m-Y h:i A') }}
                                    @else
                                ...
                                <td class="text-center">
                                    <a class="btn btn-soft-primary btn-icon btn-circle btn-sm"
                                        href="{{ route('notices.show', $notice->slug) }}"
                                    ...
                                    <a href="#"
                                        ...
                                        data-href="{{ route('notices.destroy', $notice->id) }}"
                                        title="{{ ('Delete') }}">
