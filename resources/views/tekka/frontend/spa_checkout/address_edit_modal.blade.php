...
<form class="form-default" role="form" action="{{ route('addresses.update', $address_data->id) }}" method="POST" x-data="app({{ json_encode($address_data) }})" x-cloak>
    ...
    <div class="p-3">
        ...
        <div class="row align-items-center mb-3">
            ...
            <div class="col-md-9">
                <input class="form-control" placeholder="Enter your name" name="name" minlength="3" required value="{{ $address_data->name }}">
            </div>
        </div>
        <div class="row align-items-center mb-3">
            <div class="col-md-3">
            ...
            <div class="col-md-9">
                <select class="form-control aiz-selectpicker" name="state_id" id="edit_state"  data-live-search="true" @change="handleStateChange($event.target)" required>
                    @foreach ($states as $state_id => $state_name)
                        <option value="{{ $state_id }}" @if($address_data->state_id == $state_id) selected @endif>
                            {{ $state_name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="col-md-9">
                <select class="form-control aiz-selectpicker" data-live-search="true" name="city_id" id="edit_city" @change="handleCityChange($event.target)" required>
                    @foreach ($cities as $city_id => $city_name)
                        <option value="{{ $city_id }}" @if($address_data->city_id == $city_id) selected @endif>
                            {{ $city_name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="col-md-9">
                <select class="form-control aiz-selectpicker" data-live-search="true" id="edit_area" name="area_id" required>
                    @foreach ($areas as $area_id => $area_name)
                        <option value="{{ $area_id }}" @if($address_data->area_id == $area_id) selected @endif>
                            {{ $area_name }}
                        </option>
                    @endforeach
                </select>
        ...
        <div class="form-group text-center">
            <button x-bind:type="(editMobileNumber.length < 11 || editMobileNumber.length > 11) ? 'button' : 'submit'" class="btn btn-sm btn-dark" x-bind:disabled="(editMobileNumber.length < 11 || editMobileNumber.length > 11)">{{ ('Update')}}</button>
        </div>
    </div>
</form>
