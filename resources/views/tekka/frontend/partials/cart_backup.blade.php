    ...
    @if(isset($cart) && count($cart) > 0)
        ...
        <ul class="h-250px overflow-auto c-scrollbar-light list-group list-group-flush">
            ...
            @foreach($cart as $key => $cartItem)
                @php
                    ...
                    $total = $total + $absolutePrice * $cartItem['quantity'];
                @endphp
                @if ($product != null)
                        ...
                        <span class="d-flex align-items-center">
                            <a href="{{ route('product', $product->slug) }}" class="text-reset d-flex align-items-center flex-grow-1">
                                ...
                                <span class="minw-0 pl-2 flex-grow-1">
                                    ...
                                    <span class="">{{ single_price($absolutePrice) }}</span>
                                </span>
                            </a>
                            <span class="">
        ...
        <div class="px-3 py-2 text-center border-top">
            <ul class="list-inline mb-0">
                ...
                @if (Auth::check())
                <li class="list-inline-item">
                    <a href="{{ route('checkout.shipping_info') }}" class="btn btn-black-white btn-sm">
                        {{ ('Checkout')}}
                    </a>
                </li>
                @endif
