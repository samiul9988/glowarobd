                ...
                <table class="table aiz-table mb-0">
                    <thead>
                        <tr>
                            ...
                            <th data-breakpoints="md">{{ ('Campaign Count') }}</th>
                    ...
                    <tbody>
                        @foreach ($categories as $key => $category)
                            <tr>
                                ...
                                <td>
                                    <a href="{{ route('campaigns.index',['category'=>$category->id]) }}" class="font-weight-bold">{{ $category->campaigns->count() }}</a>
                                </td>
                                <td>
                                ...
                                <td class="text-center">
                                    ...
                                    <a href="#" class="btn btn-soft-danger btn-icon btn-circle btn-sm confirm-delete"
                                        data-href="{{ route('campaign-categories.destroy', $category->id) }}"
                                        @if($category->campaigns->count() > 0) data-message="{{ ('Deleting this category will remove '.$category->campaigns->count().' campaigns') }}" @endif
                                        title="{{ ('Delete') }}">
...
@section('script')
    <script type="text/javascript">
        ...
        function showModal(type = 'create', data = null) {
            ...
            let url = type === 'create' 
                ? '{{ route("campaign-categories.store") }}' 
                : '{{ route("campaign-categories.update", ":id") }}'.replace(':id', data.id);
        ...
        function change_status() {
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('campaign-categories.bulk-status-update') }}",
                data: {
                    ids: ids,
                    status: status,
                    _token: '{{ csrf_token() }}'
        ...
        function bulk_delete() {
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('campaign-categories.bulk-delete') }}",
                data: {
                    ids: ids,
                    _token: '{{ csrf_token() }}'
                },
