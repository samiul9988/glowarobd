    ...
    <div class="container text-left">
        <div class="row">
            <div class="col-lg-4 mt-4 mt-lg-0" id="cart_summary">
                @include(config('app.theme').'frontend.partials.cart_summary')
            </div>
...
//                    console.log(data.response_message);
        ...
        // Set Reward Point Value To Maximum
        $(document).on('change', 'input[type=radio][name=availablePoint]', function() {
            $('#reawrd_point').val($(this).val());
            console.log($('#reawrd_point').val());
        });

        // Apply Reward Point
        $(document).on("click", "#apply-reward-point",function() {
            var data = new FormData($('#apply-reward-point-form')[0]);
            $.ajax({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
                method: "POST",
                url: "{{route('checkout.apply_reward_point')}}",
                data: data,
                cache: false,
                contentType: false,
                processData: false,
        ...
        // Remove Reward Point
        $(document).on("click", "#remove-reward-point",function() {
            var data = new FormData($('#remove-reward-point-form')[0]);
            $.ajax({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
