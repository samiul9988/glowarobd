...
<div class="card">
  <div class="card-header">
      ...
      <div class="pull-right clearfix">
        ...
        <a href="{{ route('sellers.approve', $seller->id) }}" class="btn btn-circle btn-dark d-innline-block">{{ ('Accept')}}</a>
      </div>
  </div>
      ...
      <div class="col-md-5">
        <h6 class="mb-4">{{ ('Verification Info') }}</h6>
                  ...
                  @foreach (json_decode($seller->verification_info) as $key => $info)
                      <tr>
                          ...
                          @elseif ($info->type == 'file')
                              <td>
                                  <a href="{{ my_asset($info->value) }}" target="_blank" class="btn-info">{{ ('Click here')}}</a>
                              </td>
                          @endif
                      </tr>
                  @endforeach
          ...
          <div class="text-center">
              <a href="{{ route('sellers.reject', $seller->id) }}" class="btn btn-sm btn-default d-innline-block">{{ ('Reject')}}</a></li>
              <a href="{{ route('sellers.approve', $seller->id) }}" class="btn btn-sm btn-dark d-innline-block">{{ ('Accept')}}</a>
          </div>
      </div>
  </div>
</div>
