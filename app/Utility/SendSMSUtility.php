...
{
    public static function sendSMS(string $to, string $from, string $text, ?string $template_id)
    {
        ...
        elseif (OtpConfiguration::where('type', 'msegat')->first()->value == 1) {
            ...
            curl_close($ch);
            return $response;
        }elseif(OtpConfiguration::where('type', 'viatech')->first()->value == 1) {
            $apiKey = env("VIATECH_SMS_API_KEY"); //put viatech provided api_key here
            $sid = env("VIATECH_SMS_SID"); // put viatech provided sid here

            $url = env("VIATECH_SMS_URL");
            $data = [
                "api_key" => $apiKey,
                "type" => "text",
                "contacts" => $to,
