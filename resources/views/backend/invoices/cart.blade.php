                ...
                <div class="row gutters-5 align-items-center">
                    <div class="col-auto w-60px">
                        <div class="row no-gutters align-items-center flex-column aiz-plus-minus">
                            ...
                            <input type="text" name="qty-{{ $key }}" id="qty-{{ $key }}" class="col border-0 text-center flex-grow-1 fs-16 input-number inputCart-{{$cartItem['id']}}" placeholder="1" value="{{ $cartItem['quantity'] }}" min="{{ $stock->product->min_qty }}" max="{{ $stock->qty }}" onchange="updateQuantity({{ $cartItem->id }},this.value)" data-edit="true">
...
<div>
    ...
    <div class="d-flex justify-content-between fw-600 mb-2 opacity-70">
        <span>{{ ('Discount')}}</span>
        <span>{{ single_price(Session::get('invoice.discount', 0)) }}</span>
    </div>
    @php
        $paidAmount = $order->payments?->sum('amount') ?? 0;
        $total = $subtotal+$tax+$orderItems[0]->shipping_cost - (Session::get('invoice.discount', 0) + $paidAmount);
    @endphp
    @if($order->payment_status != 'unpaid')
    <div class="d-flex justify-content-between fw-600 mb-2 opacity-70">
        <span>{{ ('Paid Amount')}}</span>
        <span>{{ single_price($paidAmount) }}</span>
    </div>
    @endif
