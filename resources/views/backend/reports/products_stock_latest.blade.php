    ...
    <div class="card-body">
        <table class="table mb-0" id="theTable">
            ...
            <tbody>
                ...
                @php
                    if(@$item->last_closing_stock !== NULL){
                        $openingStock = ($item->last_current_plus_adjustment + $item->last_current_purchase) - ($item->last_current_sell + $item->last_current_minus_adjustment) + $item->last_closing_stock;
                    }else{
                ...
                <tr>
                    <td>{{$key + 1 }}</td>
                    <td style="background-color: #efefef">{{@$item->name}} @if(@$item->variant != NULL)({{@$item->variant}})@endif</td>
