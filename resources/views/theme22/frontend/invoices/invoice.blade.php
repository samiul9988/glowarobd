		...
		<div style="padding: 1rem;padding-top: 0; padding-bottom: 0">
			<table>
				<tr>
					...
					<td class="text-center mx-auto" style="width: 30%;">
						{{-- @if(isset($order->user->customeringroup->group->group_image))
						<img
							class="img-fit lazyload text-center mx-auto"
							src="{{ uploaded_asset(@$order->user->customeringroup->group->group_image) }}"
							alt="{{ @$order->user->customeringroup->group->group_name }}"
							style="width: 50px;"
						>
						@endif --}}
						<h1 class="text-center mx-auto" style="text-transform: uppercase;">{{ @$order->user->customeringroup->group->group_name }}</h1>
					</td>
				</tr>
			</table>
		</div>
						...
						<td style="text-align: left; vertical-align: top">
							<table class="text-left sm-padding small strong" style="width: 80%">
								<tbody>
									...
									<tr>
										<th class="gry-color text-left">{{ ('Delivery Date') }}</th>
										<td class="currency no-padding">
						...
						<td style="text-align: left; vertical-align: top">
							<table class="text-right sm-padding small strong" style="width: 50%;">
								<tbody>
									...
									<tr class="border-bottom">
										<th class="gry-color text-left">{{ ('Paid Amount') }}</th>
										<td class="currency no-padding" style="padding: 0px!important">{{ single_price($paidAmount) }} (-)</td>
									</tr>
									@endif
									<tr>
										<th class="text-left strong"><strong>{{ ('Net Total') }}</strong></th>
										<td class="currency no-padding" style="padding: 0px!important">{{ single_price(max(0, $total)) }}</td>
									</tr>
								</tbody>
							</table>
						</td>
				...
				<table class="padding text-left small border-bottom">
					<thead>
						<tr class="gry-color" style="background: #eceff4;">
							...
							<th width="50%" class="text-left">{{ ('Payment Method')}}</th>
							<th width="40%" class="text-left">{{ ('Amount')}}</th>
						</tr>
					</thead>
					<tbody class="strong">
						@foreach ($order->payments as $key => $payment)
							<tr>
								...
								<td class="currency">{{ single_price($payment->amount) }}</td>
							</tr>
						@endforeach
					</tbody>
				</table>
