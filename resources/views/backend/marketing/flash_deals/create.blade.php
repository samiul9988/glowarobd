    ...
    <script type="text/javascript">
        (function() {
            ...
            const templates = {
                ...
                productRow: (product) => `
                        <tr id="product-row-${product.id}" style="display: none;" data-product-id='${product.id}'>
                        ...
                        <td>
                            <input type="number" lang="en" name="discounts[]" value="${product.discount ?? 0}" min="0" step="1" class="form-control form-control-sm discount-input" required>
                        </td>
                        <td>
                        ...
                        <td>
                            <input type="number" name="quantities[]" min="0" data-max="${Math.max(product.qty ?? 0, 0)}" value="${product.qty ?? 0}" step="1" class="form-control form-control-sm product-quantity" required>
                            <small class="text-muted ">Current Stock: ${product.qty ?? 0}</small>
                        </td>
                        <td class="text-center">
                        ...
                        <input type="hidden" name="current_stocks[]" value="${product.qty ?? 0}">
                    </tr>`
            };
            ...
            async function handleProductSelection(e) {
                ...
                try {
                    ...
                    for (const id of newlySelected) {
                        ...
                        // Check if this product exists in another deal
            ...
            // Initialize Bootstrap Select with AJAX
            ...
            async function init() {
                ...
                // Discount change listeners: use 'input' for numeric input and 'change' for select.
                // Also listen to selectpicker's changed.bs.select to catch UI plugin events.
