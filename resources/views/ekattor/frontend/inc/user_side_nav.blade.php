<div class="aiz-user-sidenav-wrap position-relative z-1 shadow-sm">
    <div class="aiz-user-sidenav rounded overflow-auto c-scrollbar-light pb-5 pb-xl-0">
        <div class="p-4 text-xl-center mb-4 border-bottom bg-primary text-white position-relative">
            ...
            <h4 class="h5 fs-16 mb-1 fw-600">
                {{ Auth::user()->name }}


                @if(@$user_group->group->group_image != '')
                    <img width="25" style="vertical-align:text-top" src="{{ uploaded_asset($user_group->group->group_image)}}" alt="{{$user_group->group->group_name}}">
                @else

                    @if(@$user_group->group->group_icon != '')
                        {!! $user_group->group->group_icon !!}
                    @else
        ...
        <div class="sidemnenu mb-3">
            <ul class="aiz-side-nav-list px-2" data-toggle="aiz-side-menu">
                ...
                @else
                    ...
                    @php
                        $notifications_unread = unread_notification();
                    @endphp
                    <li class="aiz-side-nav-item">
                        <a href="{{ route('user_notification.notifications') }}" class="aiz-side-nav-link {{ areActiveRoutes(['user_notification.notifications'])}}">
                            <i class="las la-bell aiz-side-nav-icon"></i>

                            <span class="aiz-side-nav-text">{{translate('Notifications')}}</span>
                            @if($notifications_unread > 0)<span class="badge badge-inline badge-success notification_unread">{{ $notifications_unread }}</span> @endif
                        </a>
                    </li>

                    @if(get_setting('reward_point_system') == 1)
                    <li class="aiz-side-nav-item">
                        <a href="{{ route('user.reward.log') }}" class="aiz-side-nav-link  {{ areActiveRoutes(['user.reward.log'])}}">
                            <i class="las la-file-alt aiz-side-nav-icon"></i>
                            <span class="aiz-side-nav-text">{{translate('Reward Point History')}}</span>  
                        </a>
                    </li>
                    @endif
