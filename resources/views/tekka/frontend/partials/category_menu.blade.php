@php
    ...
    if (file_exists($categoryfilePath)) {
        ...
        $categories = collect(json_decode($jsonData, true)) ?? collect();
...
<div class="aiz-category-menu bg-white rounded @if (Route::currentRouteName() == 'home') shadow-sm" @else shadow-lg" id="category-sidebar" @endif>
    <div class="p-2
    d-none d-lg-block rounded-top all-category position-relative text-left text-white">
...
<ul class="list-unstyled categories no-scrollbar py-2 mb-0 text-left">
    @php
        // dd($categories->where('level', 0)->reverse()->take(11));
    @endphp
    @foreach ($categories->where('level', 0)->sortByDesc('order_level')->take(11)->reverse() as $key => $category)
        @php 
        ...
        <li class="category-nav-element" data-id="{{ $category->id }}">
            <a href="{{ route('products.category', $category->slug) }}" class="text-truncate py-2 px-3 d-block">
                <img class="cat-image lazyload mr-2 opacity-60" src="{{ static_asset('assets/img/placeholder.jpg') }}"
                    data-src="{{ uploaded_asset($agent->isMobile() ? ($icon['mobile'] ?? '') : ($icon['web'] ?? '')) }}" width="16" alt="{{ $category->name }}"
            ...
            @if (count(\App\Utility\CategoryUtility::get_immediate_children_ids($category->id)) > 0)
                <div class="sub-cat-menu c-scrollbar-light rounded shadow-lg p-4">
                    ...
                    @include(config('app.theme') . 'frontend.partials.category_elements', [
                        'category' => $category,
