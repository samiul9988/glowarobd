...
class SSLCommerz
{
    ...
    public function orderValidate($trx_id = '', $amount = 0, $currency = "BDT", $post_data)
    {
        ...
        if ($validation) {
            ...
            Log::channel('custom')->error("SSLCommerz payment validation failed", [
                'trx_id' => $trx_id,
                'amount' => $amount,
                'currency' => $currency,
                'post_data' => $post_data
    ...
    protected function validate($merchant_trans_id, $merchant_trans_amount, $merchant_trans_currency, $post_data)
    {
        ...
        if ($merchant_trans_id != "" && $merchant_trans_amount != 0) {
            ...
            // dd('SSLCommerz Validation url: '. $this->sslc_validation_url);
            Log::channel('custom')->info('SSLCommerz Validation url: '. $this->sslc_validation_url);
            try {
                    ...
                    if ($status == "VALID" || $status == "VALIDATED") {
                        if ($merchant_trans_currency == "BDT") {
                            if (trim($merchant_trans_id) == trim($tran_id) && (abs($merchant_trans_amount - $amount) < 1) && trim($merchant_trans_currency) == trim('BDT')) {
                                Log::channel('custom')->info('SSLCommerz Transaction Verified');
                                return true;
                            } else {
                                # DATA TEMPERED
                                $this->error = "Data has been tempered";
                                Log::channel('custom')->info('SSLCommerz Error: '."Data has been tempered");
                                return false;
                            }
                        } else {
                        ...
                        Log::channel('custom')->info('SSLCommerz Error: '."Failed Transaction");
                        return false;
                    }
                } else {
                    # Failed to connect with SSLCOMMERZ
                    $this->error = "Failed to connect with SSLCOMMERZ";
                    Log::channel('custom')->info('SSLCommerz Error: '."Failed to connect with SSLCOMMERZ");
                    return false;
                }

            } catch (\Throwable $th) {
                Log::channel('custom')->info('SSLCommerz Error: '.$th->getMessage());
                throw $th;
            }
        } else {
            # INVALID DATA
