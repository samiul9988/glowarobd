    ...
    <div class="shadow-sm bg-white p-3 p-lg-0 rounded text-left">
        <div>
            <div class="row d-none d-lg-flex border-bottom mb-3 p-3 bg-primary text-white mx-0">
                ...
                <div class="col fw-600 text-center">{{ ('Total')}}</div>
                    ...
                    <li class="list-group-item px-0 px-lg-3">
                        <div class="row gutters-5">
                            ...
                            <div class="col-lg col-4 order-1 order-lg-0 my-3 my-lg-0">
                                ...
                                <span class="text-reset badge badge-inline badge-soft-secondary">Saved {{ single_price($savedAmount) }}</span>
                                @endif
                            </div>
                            ...
                            <div class="col-lg text-left col-6 order-4 order-lg-0">
                                @if($cartItem['digital'] != 1 && $product->auction_product == 0)
                                    <div class="row no-gutters align-items-center aiz-plus-minus mr-2 ml-0">
                                        <button class="btn col-auto btn-icon btn-sm btn-circle btn-light" type="button" data-id="{{ $cartItem['id'] }}" data-type="minus" data-field="quantity[{{ $cartItem['id'] }}]" @click="decreaseQuantity({{ $cartItem['id'] }})">
                                            <i class="las la-minus"></i>
                                        </button>
                                        <input type="number" name="quantity[{{ $cartItem['id'] }}]" class="col border-0 text-center flex-grow-1 fs-16 input-number inputCart-{{ $cartItem['id'] }}" placeholder="1" value="{{ $cartItem['quantity'] }}" min="{{ $product->min_qty }}" max="{{ $max_limit }}" step="1">
                                        <button class="btn col-auto btn-icon btn-sm btn-circle btn-light" type="button" data-id="{{ $cartItem['id'] }}" data-type="plus" data-field="quantity[{{ $cartItem['id'] }}]" @click="increaseQuantity({{ $cartItem['id'] }})">
        ...
        <div class="p-md-4">
            @if($sDiscount && $sDiscount['status'] && $sDiscount['min_amount'] > 0)
                @php
                    $sDiff = $sDiscount['min_amount'] - $total;
                @endphp
                <span class="text-success text-capitalize">@if($sDiff > 0)Add More <strong>{{single_price($sDiff)}}</strong> to Get <strong>{{single_price($sDiscount['amount'])}}</strong> Shipping Fee Offer @else You're Enjoying <strong>{{single_price($sDiscount['amount'])}}</strong> Shipping Fee Offer @endif</span>
