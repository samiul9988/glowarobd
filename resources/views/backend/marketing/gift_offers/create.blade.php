    ...
    <div class="col-lg-10 mx-auto">
        <div class="card">
            <div class="card-header">
                <h5 class="mb-0 h6">Gift Offer Information</h5>
            </div>
            <div class="card-body">
                <form action="{{ route('admin.gift_offers.store') }}" method="POST" id="gift_offer_form">
                    @csrf
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-9">
                            <input type="text" placeholder="Enter offer title" id="title" name="title" class="form-control" value="{{ old('title') }}" required>
                        </div>
                    </div>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-9">
                            <textarea name="description" id="description" class="form-control" rows="3" placeholder="Enter description (optional)">{{ old('description') }}</textarea>
                        </div>
                    </div>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-9">
                            <select name="offer_type" id="offer_type" class="form-control aiz-selectpicker" required onchange="toggleConditionFields()">
                                ...
                                <option value="product" {{ old('offer_type') == 'product' ? 'selected' : '' }}>Product Wise - Free gift when specific product is in cart</option>
                                <option value="cart" {{ old('offer_type') == 'cart' ? 'selected' : '' }}>Cart Amount - Free gift when cart total reaches minimum amount</option>
                            </select>
                        </div>
                    </div>

                    {{-- Cart Amount Conditions (for cart_amount type) --}}
                    <div class="form-group row" id="cart_amount_section" style="display: none;">
                        <label class="col-sm-3 control-label">Cart Amount Range</label>
                        <div class="col-sm-9">
                            <div class="row">
                                <div class="col-md-12">
                                    <label>Minimum Amount <span class="text-danger">*</span></label>
                                    <input type="number" name="min_cart_amount" id="min_cart_amount" class="form-control" placeholder="0" step="1" min="0" value="{{ old('min_cart_amount', 0) }}">
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-group row" id="product_condition_section" style="display: none;">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-9">
                            <input type="text" class="form-control aiz-date-range" name="date_range" placeholder="Select Date Range" data-time-picker="true" data-format="DD-MM-Y HH:mm:ss" data-separator=" to " autocomplete="off" required>
                        </div>
                    </div>
                        ...
                        <div class="col-sm-9">
                            <div class="row">
                                <div class="col-md-6">
                                    ...
                                    <input type="number" name="max_item_per_order" id="max_item_per_order" class="form-control" value="{{ old('max_item_per_order', 1) }}" min="1" required>
                                </div>
                                <div class="col-md-6">
                                    <label>
                                    ...
                                    <input type="number" name="max_qty_per_order" id="max_qty_per_order" class="form-control" value="{{ old('max_qty_per_order', 1) }}" min="1" required>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-group mt-4">
                        <table class="table table-bordered">
                            <thead>
                                <tr>
                                    <th width="30%">Gift Product</th>
                                    <th width="12%" class="text-center">Current Stock</th>
                                    <th width="12%" class="text-center">
                                        Allocate Qty
                                        @include('components.tooltip', ['title' => 'Number of units available for this gift offer'])
                                    </th>
                                    <th width="12%" class="text-center">Original Price</th>
                                    <th width="14%" class="text-center">
                                        Offer Price
                                        @include('components.tooltip', ['title' => 'Discounted price for the gift (0 = FREE)'])
                                    </th>
                    ...
                    <div class="form-group mb-0 text-right">
                        <button type="submit" class="btn btn-primary">{{ 'Save Gift Offer' }}</button>
                    </div>
                </form>
            </div>
        </div>
...
<script type="text/javascript">
    (function() {
        ...
        function initProductSearch(selectElement, onSelectCallback) {
            ...
            // Listen to search input in dropdown
        ...
        function addGiftProduct(productId, product) {
            ...
            state.selectedGiftProducts.set(productId, product);
        ...
        function renderGiftProductsTable() {
            ...
            state.selectedGiftProducts.forEach((product, productId) => {
                ...
                const thumbnail = product.image_url || '{{ static_asset('assets/img/placeholder.jpg') }}';
                ...
                DOM.giftProductsTable.append(`
                    <tr data-id="${productId}">
                        ...
                        <td class="text-center">
                            <input type="number" name="gift_quantities[]" class="form-control form-control-sm text-center gift-quantities" value="${Math.max(currentStock, 0)}" min="1" max="${currentStock}" required>
                        </td>
                        <td class="text-center">
        ...
        function addConditionProduct(productId, product) {
            ...
            state.selectedConditionProducts.set(productId, product);
            ...
            state.selectedConditionProducts.forEach((product, productId) => {
                html += `
                    <tr>
                        ...
                        <td>
                            <input type="number" name="condition_min_qty[]" class="form-control form-control-sm" value="1" min="1">
                        </td>
                        <td class="text-center">
        ...
        $(document).ready(function() {
            ...
            // Initialize gift products search
            initProductSearch(DOM.giftProductsSelect, addGiftProduct);

            // Initialize condition products search
