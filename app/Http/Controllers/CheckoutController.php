...
class CheckoutController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $addresses = Address::with('area:id,name', 'city:id,name', 'state:id,name')->where($request->user_field, $userId)->orderBy('set_default', 'desc')->orderBy('updated_at', 'desc')->get();
        ...
        // dd($carts, $cartShippingCharge, $addresses, $shippingMethods, $selectedAddress);
    ...
    public function get_shipping_methods(Request $request)
    {
        ...
        if (filled($request->address_id)) {
            $selectedAddress = Address::with('area:id,name', 'city:id,name', 'state:id,name')->where($request->user_field, $userId)->find($request->address_id);
            $areaId = $selectedAddress->area_id;
        } else {
        ...
        if (!empty($sDiscount) && data_get($sDiscount, 'status', false) == true) {
            ...
            if ($minAmount <= $carts->sum('price')) {
                ...
                $remaining = single_price($minAmount - $carts->sum('price'));
    ...
    public function checkout(Request $request)
    {
        if($request->address_id == 0 || empty($request->address_id)){
            ...
            if (!empty(array_filter($addressFields))) {
                DB::beginTransaction();
                try{
                    $address = Address::create(array_merge($addressFields, [
                        'user_id' => Auth::id(),
                        'set_default' => 0,
                    ]));
                    $user = Auth::user();
                    if($user && strtolower(str_replace(' ','',$user->name)) === 'newuser('.$user->phone.')'){
                        $user->name = $address->name ?? $user->name;
                        $user->save();
                    }
                    DB::commit();
    ...
    public function get_shipping_info(Request $request)
    {
        ...
        // Remove all gift offer carts
        Cart::withoutGlobalScopes()->withoutRegular()->where($request->user_field, $userId)->delete();
                ...
                if ($pass) {
                    ...
                    if ($coupon->type == 'cart_base') {
                        ...
                        if ($cartTotal < $coupon_details->min_buy) {
                            ...
                            $response_message['response'] = 'danger';
                            $response_message['message'] = "Add more " . single_price($needToBuyMore) . " to apply this coupon!";
                            $shipping_info = Address::where('id', $carts[0]['address_id'])->first();
                            $sDiscount = array();
                            $shipping_charge = $carts->sum('shipping_cost');
                            if (get_setting('spa_checkout') == 1){
                    ...
                    if ($cart) {
                        ...
                        $response_message['response'] = 'danger';
                        $response_message['message'] = ('No eligible items in cart for this coupon!');
                        $response_message['type'] = 'coupon';
                        $couponApplied = false;
                    }
                } else {
