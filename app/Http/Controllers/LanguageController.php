...
class LanguageController extends Controller
{
    public function changeLanguage(Request $request)
    {
    	...
    	flash(('Language changed to ').$language->name)->success();
    }
    ...
    public function store(Request $request)
    {
        ...
        if (!file_exists($languageFilePath) || file_exists($languageFilePath)) {
    ...
    public function update(Request $request, $id)
    {
        ...
        if (env('DEFAULT_LANGUAGE') == $language->code) {
            flash(('Default language can not be edited'))->error();
            return back();
        }
        $language->name = $request->name;
        $language->code = $request->code;
    ...
    public function key_value_store(Request $request)
    {
        ...
        flash(('Translations updated for ').$language->name)->success();
        return back();
    }
    ...
    public function update_rtl_status(Request $request)
    {
        ...
        if($language->save()){
            flash(('RTL status updated successfully'))->success();
            return 1;
        }
        return 0;
    }
    ...
    public function destroy($id)
    {
        $language = Language::findOrFail($id);
        if (env('DEFAULT_LANGUAGE') == $language->code) {
            flash(('Default language can not be deleted'))->error();
        }
        else {
            ...
            flash(('Language has been deleted successfully'))->success();
        }
        return redirect()->route('languages.index');
    }
    ...
    public function importEnglishFile(Request $request){
        ...
        flash(('Translation keys has been imported successfully. Go to App Translation for more..'))->success();
        return back();
    }
    ...
    public function storeAppTranlsation(Request $request){
        ...
        flash(('App Translations updated for ').$language->name)->success();
        return back();
    }
