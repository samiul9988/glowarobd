...
import PriceRangeBrandSelection from "./PriceRangeBrandSelection";

const FilterBrandWidgets = () => {
  // Rating filtering
  const { rating, brand, category, setFilter } = useBrandStore();
  ...
  const handleRatingSelect = (value: number | null) => {
    ...
    setFilter("page", 1); // reset pagination
  };
  ...
  const { brand_id, setFilter: setBrandFilter } = useBrandStore();
  ...
  const handleBrandSelect = (slug: string) => {
    ...
    setBrandFilter("brand_id", newValue);
    setBrandFilter("page", 1); // reset pagination
  };
      ...
      <div className="space-y-4 lg:space-y-14">
        ...
        >
          <AccordionItem value="brand">
            ...
            <AccordionContent className="scroll-thin flex max-h-[450px] flex-col gap-1 overflow-y-auto pt-3 text-balance md:pt-6">
                ...
                <>
                  {allBrands.data.map((brand: BrandItemResponse) => (
                    ...
                    >
                      <Checkbox
                        id={brand.slug}
                        checked={params?.brand_slug === brand.slug}
...
export default FilterBrandWidgets;
