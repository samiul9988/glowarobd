...
class PaymentInfoMail extends Mailable implements ShouldQueue
{
    ...
    {
        $this->info = $info;
        $this->template = MailTemplate::where('type', 'Payment Confirmation')->where('status', 1)->first();

        if(!$this->template || is_null($this->template->content)){
            return null;
        }
    }
    ...
    private function prepareSubject()
    {
        ...
        return '💳 Payment Confirmation for Order #' . ($this->info['order_id'] ?? '');
    }
    ...
    private function prepareContent()
    {
        if ($this->template && $this->template->content) {
            ...
            $this->dynamicVariables =[
                ...
                "payment_amount" => $this->info['amount'] ?? '',
                "payment_date" => $this->info['payment_date'] ?? '',
                "payment_status" => '<span style="color:green; font-weight:bold;">'.$this->info['status'].'</span>',
