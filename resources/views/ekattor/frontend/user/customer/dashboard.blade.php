...
<div class="row gutters-10">
    <div class="col-md-12">
        <div class="card">
            <div class="card-header">
                <h6 class="mb-0">{{ translate('My Groups') }}</h6>
            </div>
            <div class="card-body">
            @php
            ...
            @endphp
                ...
                <div class="customer-group-dashboard d-flex flex-wrap justify-content-center customer-group-flex">

                    @foreach($groups as $group)
                        <div class="card mx-2 position-relative @php if(@$user_group->group->group_name == @$group->group_name){
                                    echo "group-opacity"; }@endphp">
                            <div class="customer-group-level @php if(@$user_group->group->group_name == @$group->group_name){
                                    echo "current-customer-group-level h"; }@endphp">Lvl {{$group->ordering}}
                            </div>
                            <i data-toggle="modal" data-target="#CustomerGroup{{@$group->id}}" class="las la-info-circle @php if(@$user_group->group->group_name == @$group->group_name){
                                    echo "group-opacity"; }@endphp"></i>



                            <div class="card-body pb-0
                            @php if(@$user_group->group->group_name == @$group->group_name){
                                echo "bg-grad-2"; }@endphp">
                                ...
                                <p class="text-center @php if(@$user_group->group->group_name == @$group->group_name){
                                echo "current-customer-group-name"; }@endphp" style="color:#606060">{{@$group->group_name}}</p>
                            </div>
                        </div>
                        <!-- Modal -->
