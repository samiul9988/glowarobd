        ...
        <table class="table mb-0" id="theTable">
            ...
            <tbody>
                ...
                <tr>
                    <td>{{@$entry->date}}</td>
                    @php
                    if(filled(@$entry->current_opening_stock)){
                        $openStock = $entry->current_opening_stock;
                    }else{
                        $openStock = (@$entry->opening_purchase + @$entry->opening_plus_adjustment) - (@$entry->opening_sell + @$entry->opening_minus_adjustment);
                    }
                    // dump('Opening Stock: '.$openStock);
                    @endphp
                    <td class="text-right" data-val="{{$openStock}}">
                        {{$openStock}}
                    </td>
                    ...
                    @php
                    $closeStock = $openStock + $entry->purchases - $entry->sales + $adjustments;
                    // dump('Closing Stock: '.$closeStock);
                    @endphp
                    <td class="text-right" data-val="{{ $closeStock }}">
                        {{ $closeStock }}
                    </td>
