...
<div class="aiz-card-box border border-light rounded hov-shadow-md mt-1 mb-1 has-transition bg-white product_box">
    @if($product['has_discount'] === true)
        <div class="ribbon"><span class="ribbon4"><span>{{ ('OFF') }}&nbsp;{{get_discount_percentage($product['stroked_price'], $product['main_price'])}}%</span></span></div>
    @endif
    <div class="position-relative p-2 px-md-2">
        <a href="{{ route('product', $product['slug']) }}" class="d-block">
            <img
                class="img-fit card-image lazyload mx-auto product_long_grid"
