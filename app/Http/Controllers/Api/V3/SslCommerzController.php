...
class SslCommerzController extends Controller
{
    ...
    public function begin(Request $request)
    {
        ...
        if ($request->payment_type == "cart_payment") {
            ...
            if ($request->header('source', 'app') == 'web') {
                ...
                $post_data['value_b'] = $request->combined_order_id . '-JOIN-' . $order->id;
            } else {
        ...
        $post_data['success_url'] = url("api/v3/sslcommerz/success");
        $post_data['fail_url'] = url("api/v3/sslcommerz/fail");
        $post_data['cancel_url'] = url("api/v3/sslcommerz/cancel");
    ...
    public function payment_success(Request $request)
    {
        ...
        if (isset($request->value_c)) {
            try {
                $value_b = explode('-JOIN-', $request->value_b);
                $order_id = isset($value_b[1]) ? $value_b[1] : null;
                if ($request->value_c == 'cart_payment') {
                    checkout_done($value_b[0], $payment);
                } elseif ($request->value_c == 'wallet_payment') {
                    $amount = $value_d['amount'] ?? 0;
                    wallet_payment_done($request->value_a, $amount, 'SslCommerz', $payment);
                }

                if($order_id) {
                    return redirect()->away(env('FRONTEND_URL')."/order-success/{$order_id}");
                }
        ...
        if($order_id) {
            $this->managePayment($request, $order_id);
            return redirect()->away(env('FRONTEND_URL')."/order-success/{$order_id}");
        }
    ...
    private function managePayment(Request $request, $order_id)
    {
        try {
            Log::channel('custom')->info('SslCommerz Payment Success Callback', ['request' => $request->all()]);
            $order = Order::find($order_id);
            if($order && $order->payment_status == 'paid'){
                Log::channel('custom')->info('SslCommerz Payment Success Callback: Order already paid', ['order_id' => $order->id]);
                return;
            }
            ...
            $pdetails = [
                ...
                'bank_type' => 'SslCommerz Payment',
                'bank_info' => 'SslCommerz Payment',
            ...
            DB::rollBack();
            Log::error('SslCommerz Payment Success Handling Error: ' . $e->getMessage());
        }
    }
